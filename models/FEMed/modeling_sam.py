...
from .adapter_ViT import FeatureSelectionLayer, PoolingPyramidAdapterLayer, PromptGenerator, SwitchSelectionLayer
...
class FESamVisionEncoder(nn.Module):
    def __init__(self, config: SamVisionConfig):
        ...
        self.prompt_generator = PromptGenerator(self.scale_factor, self.prompt_type, config.hidden_size, #self.embed_dim,
                                                self.tuning_stage, self.depth,
                                                self.input_type, self.freq_nums,
                                                self.handcrafted_tune, self.embedding_tune, 'adaptor',
                                                self.image_size, config.patch_size)
        self.pyramid_adapter = PoolingPyramidAdapterLayer()
        self.feature_selectors = nn.ModuleList([
            SwitchSelectionLayer(config.hidden_size) for _ in range(config.num_hidden_layers)          
        ])
    ...
    def forward(
        ...
        ##! begin region FFT Adaptor ##
        embedding_feature = self.prompt_generator.init_embeddings(x)      #shape: torch.Size([1, 4096, 24])
        handcrafted_feature = self.prompt_generator.init_handcrafted(pixel_values) #shape: torch.Size([1, 24, 64, 64])
        prompt = self.prompt_generator.get_prompt(handcrafted_feature, embedding_feature) #prompt: List length=12
        ##! end region FFT Adaptor ##  
        pyramid_feature = self.pyramid_adapter(x)      
        ...
        for i, (layer_module, selector) in enumerate(zip(self.layers, self.feature_selectors)):
            
            frequency_feature = prompt[i].reshape(B, H, W, -1)
            weights = selector(x)
            ##! begin region FFT Adaptor Switch ##
            x = weights[:, 0:1] * pyramid_feature + weights[:, 1:2] * frequency_feature + x
            
            # x = frequency_feature + x
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(x, output_attentions=output_attentions)
...
class SamModel(SamPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vision_encoder = FESamVisionEncoder(config.vision_config)
